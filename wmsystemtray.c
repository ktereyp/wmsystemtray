...
static int icons_per_row=100;
static int icons_per_col=1;
...
static XSizeHints location;

static XWindowAttributes size_win;
...
static void config(){
    ...
    char filePath[100];
    sprintf(filePath, "%s/.wmsystemtray.rc", homedir);
    ...
    if(configFile != NULL){
        ...
        while(fgets(line, sizeof(line), configFile)){
            ...
            key = strtok(line, " ");
            value = strtok(NULL, " ");
            if(strncmp("X", key, 1) == 0){
                location.x = atoi(value);
            }
            if(strncmp("Y", key, 1) == 0){
                location.y = atoi(value);
            }
        }
        fclose(configFile);
    }else{
...
static void sighandler(int i){
    switch(i){
      case SIGINT:
      case SIGQUIT:
      case SIGKILL: // Useless, but... I now use it
          save();
          exit(0);
          break;
      case SIGPIPE:
...
redo:
    ...
    for(i = 0; i<num_windows; i++){
        /*update dock size*/
        XGetWindowAttributes(display, iconwin[i], &size_win);
        XSizeHints l;
        l.max_width= num_mapped_icons * iconsize + 18;
        XSetWMNormalHints(display, iconwin[i], &l);
        XMoveResizeWindow(display, iconwin[i], location.x - l.max_width, location.y, l.max_width, location.height);
...
static void create_dock_windows(int argc, char **argv){
    ...
    sizeHints->x = 0;
    sizeHints->y = 0;
    sizeHints->width = iconsize + 8;//64;
    sizeHints->height = iconsize + 8;//64;
    sizeHints->base_width = sizeHints->width;
    sizeHints->base_height = sizeHints->height;
    sizeHints->min_width = iconsize + 8;//64;
    sizeHints->min_height = iconsize + 8;//64;
    sizeHints->max_width = sizeHints->min_width;
    sizeHints->max_height = sizeHints->min_height;//64;
    warn(DEBUG_DEBUG, "Parsing geometry string '%s'", geometry);
    warn(0, "Parsing geometry string '%s'", geometry);
    ...
    if(location.x == 0)
        location.x = sizeHints->x;
    if(location.y == 0)
        location.y = sizeHints->y;
    location.width = sizeHints->width;
    location.height = sizeHints->height;

    for(int i=0; i<num_windows; i++){
        if(nonwmaker){
            iconwin[i] = XCreateSimpleWindow(display, root, location.x, location.y, location.width, location.height, 1, fgpix, bgpix);
        } else {
...
static void usage(int exitcode){
    ...
    fprintf(stderr, "  -geometry <geom>      Initial window geometry,like \"1366x768+1000+680\"\n");
    ...
    fprintf(stderr, "when running, user could drag the window, and adjust slightly by using midst \nbutton of mouse when cursor on left or right\n");
    exit(exitcode);
}
            ...
            switch(ev.type){
              ...
              case ButtonPress:
                switch (ev.xbutton.button) {
                  case 1:
                      location.x -= ev.xbutton.x;
                      location.y -= ev.xbutton.y;
                    break;
                  case 4:
                  case 6:
                    if(ev.xbutton.x < size_win.width / 2){
                        location.y += 1;
                        need_update = True;
                    }else{
                        location.x += 1;
